...
ttemplate = '''<form method="POST" action="result.py">
<input type="hidden" name="addr" value="%s"><tr>
<td align="center">%s</td><td align="right">%s</td>
<td align="center">%s</td><td><input type="number" name="channelId" value="%s"></td>
<td><input type="text" name="write_key" value="%s"></td>
<td align="center"><input type="submit" value="設定"></td>
</tr></form>'''
tbody = ''
for addr in addrs:
    d = r.hgetall(addr)
    dd = dict([(k.decode('utf-8'), v.decode('utf-8')) for k, v in d.items()])
    channelId = dd.get('channelId', '')
    if channelId == 'None': channelId = ''
    write_key = dd.get('write_key', '')
    if write_key == 'None': write_key = ''
    button = dd.get('button', '')
    if button == 'None' or button == '': button = '-'
    tbody += ttemplate % (addr, addr, dd.get('rssi'), button, channelId, write_key)
